...
class Vector:
    ...
    def __add__(self, other):
      ...
      for i in range(highest):
        if i < lowest:
          newVec.v.append(self.v[i] + other.v[i])
        else:
          if highest == x and x == r:
            newVec.v.append(self.v[i] + other.v[i])
          elif highest == x:
            newVec.v.append(other.v[i])
          else:
            newVec.v.append(self.v[i])
      return newVec
    ...
    def magnitude(self):
      x = 0
      newVec = self
      for i in range(len(newVec.v)):
        x = x + (newVec.v[i] * newVec.v[i])
      x = math.sqrt(x)
      return x
